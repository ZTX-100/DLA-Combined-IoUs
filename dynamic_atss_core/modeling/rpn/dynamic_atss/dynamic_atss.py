...
from .inference import make_dynamic_atss_postprocessor
from .loss import make_dynamic_atss_loss_evaluator

from dynamic_atss_core.layers import Scale
from dynamic_atss_core.layers import DFConv2d
from ..anchor_generator import make_anchor_generator_dynamic_atss
...
class DynamicATSSHead(torch.nn.Module):
    def __init__(self, cfg, in_channels):
        super(DynamicATSSHead, self).__init__()
        self.cfg = cfg
...
class DynamicATSSModule(torch.nn.Module):

    def __init__(self, cfg, in_channels):
        super(DynamicATSSModule, self).__init__()
        self.cfg = cfg
        self.head = DynamicATSSHead(cfg, in_channels)
        box_coder = BoxCoder(cfg)
        self.loss_evaluator = make_dynamic_atss_loss_evaluator(cfg, box_coder)
        self.box_selector_test = make_dynamic_atss_postprocessor(cfg, box_coder)
        self.anchor_generator = make_anchor_generator_dynamic_atss(cfg)
...
def build_dynamic_atss(cfg, in_channels):
    return DynamicATSSModule(cfg, in_channels)
